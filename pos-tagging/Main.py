...
class MyCbk_bi(Callback):

	def __init__(self, model):
		self.model_to_save = model

	def on_epoch_end(self, epoch, logs=None):
		self.model_to_save.save("bi.h5")

class MyCbk_dense(Callback):

	def __init__(self, model):
		self.model_to_save = model

	def on_epoch_end(self, epoch, logs=None):
		self.model_to_save.save("dense.h5")
...
full_model = Model(inputs = [model_input_source, model_input_target], outputs = [prediction])

# Define Bidirectional model

bi_model = Model(inputs = [model_input_source], outputs = [lstm_output])
...
dense_model = Model(inputs = [lstm_state, previous_tag], outputs = [decoder_prediction])
#model.save("prediction_model.h5") 



gpu_model = multi_gpu_model(full_model, gpus=4)
optimizer = RMSprop(lr=0.001)
gpu_model.compile(optimizer=optimizer, loss='categorical_crossentropy')
cbk_full = MyCbk_full(full_model)
cbk_bi = MyCbk_bi(bi_model)
cbk_dense = MyCbk_dense(dense_model)
